...
function getAggregateData(requirements, ID) {
    const aggregateData = new Map();
    for (const sheetName of requirements.keys()) {
        const sheet = fetchSheet(ID, sheetName);
...
function initEmailContentRows(emailContentRows, sheetName, status) {
...
function appendEmailContentRow(emailContentRow, sheetName, status, date) {
    const sheet = emailContentRow.get(sheetName);
    if (!sheet)
        throw new Error(`No sheet found for ${sheetName}`);
    const statusEntry = sheet.get(status);
    if (statusEntry === undefined)
        throw new Error(`No status found for ${status}`);
    sheet.set(status, statusEntry + 1);
}
function getEmailContent(requirements, aggregateData) {
    const emailContent = { standard: [], urgent: [] };
    const today = new Date();
    for (const [sheetName, statuses] of requirements) {
        const standardEmailContentRow = new Map([[sheetName, new Map()]]);
        const urgentEmailContentRow = new Map([[sheetName, new Map()]]);
        const data = aggregateData.get(sheetName);
        if (!data)
            throw new Error(`No data found for ${sheetName}`);
        const { headers, body } = data;
        if (headers.get('Status') === undefined)
            throw new Error(`No Status column found in ${sheetName}`);
        if (headers.get('Date') === undefined)
            throw new Error(`No Date column found in ${sheetName}`);
        ...
        for (let x = 0; x < statuses.length; ++x) {
            ...
            for (let y = 1; y < body.length; ++y) {
                ...
                if (entryAge >= dayThreshold)
                    appendEmailContentRow(urgentEmailContentRow, sheetName, status, dateCell);
                else
                    appendEmailContentRow(standardEmailContentRow, sheetName, status, dateCell);
            }
        }
...
function emailReportsTrigger() {
    const ID = '1TVReEBhve86gr3G6o9YVhWP2G0em9gPetxeJKkTdBDM';
    const requirements = getRequirements();
    const aggregateData = getAggregateData(requirements, ID);
    const emailContent = getEmailContent(requirements, aggregateData);
...
//# sourceMappingURL=email-reports-ss.js.map