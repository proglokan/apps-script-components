...
function initToday(localSheet: GoogleAppsScript.Spreadsheet.Sheet): number {
...
function getSourceSheet(): GoogleAppsScript.Spreadsheet.Sheet {
  const ID = '1bnihuxvO7810UWyYdyzbVGfU2ev8c64nmz1KKSJ0gIU';
  const SHEET_NAME = 'Vendor-CSV';
  const sourceWorkbook: GoogleAppsScript.Spreadsheet.Spreadsheet = SpreadsheetApp.openById(ID);
...
function getTargetColumns(headers: _Headers): number[] {
  const TARGET_HEADERS: string[] = ['VENDOR NAME', 'WEBSITE', 'EMAIL', 'PASSWORD', 'CSV', 'CD Notes', 'Check Status'];
...
function cacheCoordinates(updatedRows: number[][][]) {
...
// @arg {_Headers} headers → the headers of the source workbook
function isUpdated(size: number, data: string[][], headers: _Headers): boolean {
  const today = new Date().toLocaleDateString();
  const COLUMN_NAME = 'CSV Updated Date';
  if (headers.get(COLUMN_NAME) === undefined) throw new Error(`Header ${COLUMN_NAME} not found`);
  const column: number = headers.get(COLUMN_NAME)!;
  ...
  for (let x = 1; x < data.length; ++x) {
    ...
    if (date !== today) continue;
    const subCoordinates: number[][] = [];
    for (const y of targetColumns) subCoordinates.push([x, y]);
...
function getCoordinates(): number[][][] {
  ...
  if (value === null) throw new Error(`Cache \'${key}\' not found`);
...
function getUpdateContents(data: string[][]): (string | boolean)[][] {
  const coordinates: number[][][] = getCoordinates();
  const contents: (string | boolean)[][] = [];
